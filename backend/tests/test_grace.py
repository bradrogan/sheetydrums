...
def test_replay_reclassify_keeps_frozen_flam() -> None:
    ...
    frozen = [{"bar": 1, "note": {
        ...
        "grace": {"instrument": "tom_high", "slashed": True},
    }}]
...
def test_replay_clears_grace_when_frozen_has_none() -> None:
    # Base note carries a flam; the frozen ground truth does not → replay drops it.
    base: dict[str, Any] = {
        "version": "1", "tempo_bpm": 120.0,
        "time_signature": {"numerator": 4, "denominator": 4},
        "bars": [{"index": 1, "start_seconds": 0.0, "notes": [
